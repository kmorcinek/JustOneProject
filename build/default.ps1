...
Task Test {
    ...
    $testFolders = gci $bin_test_dir/*JustOneProject.exe
    ...
    foreach($testFolder in $testFolders) { 
        ...
        Exec {
            & $xunit $testFolderPath -notrait "Category=DoNotRun" -xml $test_output_path
        }
